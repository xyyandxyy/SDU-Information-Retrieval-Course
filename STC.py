...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', default='20ng',
                        choices=['stackoverflow', 'biomedical', 'search_snippets','20ng','Tweet','News'])
    ...
    parser.add_argument('--embedding_type', default='word2v',choices=['word2v','wiki','bert'])
    ...
    # load dataset
    ####################################################################################
    if ((args.embedding_type=='bert' or args.embedding_type=='wiki') and args.dataset in ['biomedical','stackoverflow','search_snippets']):
        print(args.dataset +"'s "+args.embedding_type+  'embedding have not implement')
        # exit(0)
    x, y = load_data(args.dataset,args.embedding_type)
    ...
    # create model
    ####################################################################################
    dec = STC(dims=[x.shape[-1], 500, 500, 200, 20], n_clusters=n_clusters)
    ...
    y_pred = dec.fit(x, y=y, tol=args.tol, maxiter=args.maxiter, batch_size=args.batch_size,
                     ...
                     rand_seed=0)
    # print(learn_rate, ' ', batch_size)
    print('acc:', metrics.acc(np.array(y), np.array(y_pred)))
    print('nmi:', metrics.nmi(np.array(y), np.array(y_pred)))
    print('homogeneity:', metrics.homogeneity(y, y_pred))
    print('completeness:', metrics.completeness(y, y_pred))
    print('v_measure:', metrics.v_measure(y, y_pred))
